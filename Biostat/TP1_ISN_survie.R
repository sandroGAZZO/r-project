...
library(maxLik)
setwd("C:/Users/Sandro/Documents/R/Biostat/")
...
#' @) Estimation  des paramètres pour loi de Weibull, Gamma et Exponentielle. Ajouter les fonctions de survie estimées sur l'estimation non-paramétrique de Kaplan-Meier
#' 
## ---------------------------------------------------------
library(fitdistrplus)
plot(result.km, conf.int=TRUE, mark="+", xlab="Time (months)", ylab ="Survival")
...
### MATERNITE ######
load(file="FlowDataAnalyse.RData", .GlobalEnv) # Lire les donn�ees
head(data) # Premi`eres lignes
str(data) # Structure de donn�ees

dur_obs = data$End_obs - data$Start_obs # Dur�ee d'observation pour chaque patiente
dur_obs = as.numeric(dur_obs)/(60*60) # Dur�ee en heures
dur_acc = data$End_lab- data$Start_lab # Dur�ee de travail pour chaque patiente
dur_acc = as.numeric(dur_acc)*60*60 # Dur�ee en heures

ggplot(data=data,aes(x=dur_obs))+geom_histogram(color="blue",fill="blue") +geom_density()
...
# Esp�erance de la loi exponentielle
mean_exp=1/rate
# Esp�erance de Weibull
scale*gamma(1+shape)
# M�ediane observ�ee
med_dur=median(dur_obs)
# M�ediane de loi exponentielle
med_exp=log(2)/rate
# M�ediane de Weibull
med_weib=scale*log(2)^(1/shape)
