...
workDir <- "C:/Users/Sandro/Documents/R/Biostat/TP_seq"      # working directory for the R session

projectName <- "DESseq2"                         # name of the project
author <- "Sandro Gazzo"                                # author of the statistical analysis/report

targetFile <- "target.txt"                           # path to the design/target file
rawDir <- "lobel2016Count"                                      # path to the directory containing raw counts files
featuresToRemove <- c("alignment_not_unique",        # names of the features to be removed
                      "ambiguous", "no_feature",     # (specific HTSeq-count information and rRNA for example)
                      "not_aligned", "too_low_aQual")# NULL if no feature to remove

varInt <- "strain"                                    # factor of interest
condRef <- "WT"                                      # reference biological condition
batch <- NULL                                        # blocking factor: NULL (default) or "batch" for example
#batch <- "medium"                                        # blocking factor: NULL (default) or "batch" for example
